...
bool PlatnySoubor(std::string soubor, std::string error = "") {
	...
	for (int i = 2; i < 102; i++) {
		...
		// Pokud poslední hodnota v řádku není rovna souctu radku % 3, radek je neplatny
		if (radek[10] - 48 != (soucetradku - radek[10] + 48) % 3) {
			std::cout << error;
			return false;
		}
	...
	std::string delka;
	for (int i = 17; i < PrectiRadek(soubor, 102).length(); i++) {
...
void UlozSoubor(std::string soubor, int PoleOdpovedi[][10]) {
	...
	int soucetradku, soucetsouboru = 0;
