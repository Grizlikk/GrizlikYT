...
// Funkce pro spusteni jednoho kola hry sam proti sobe
void HraProtiSobe(int PoleOdpovedi[][10]) {
	int pozice, vybraneodpovedi[50][4], kolo = 0, soucet = 0;

	while (true) {
		// Vybrani nahodne odpovedi prvniho hrace
		do {
			pozice = rand() % 10;
			// Platne hodnoty jsou od 1 do 10, vsechny tedy posunu o 1 pole v ASCII zpet, diky cemuz budou platne hodnoty od 0 do 9 a z cisla 0 se stane /
		} while (!ZnakVTextu("0123456789", (char)(PoleOdpovedi[soucet][pozice] + 47)));
		...
		if (soucet == 100) {
			...
			for (int i = 0; i < kolo; i++) {
				PoleOdpovedi[vybraneodpovedi[i][2]][vybraneodpovedi[i][3]] = 0;
			}
			return;
		}
		else if (soucet > 100) {
		...
		// Vybrani nahodne odpovedi druheho hrace
		do {
			pozice = rand() % 10;
		} while (!ZnakVTextu("0123456789", (char)(PoleOdpovedi[soucet][pozice] + 47)));

		// Zahrani odpovedi a ulozeni vybrane hodnoty
		vybraneodpovedi[kolo][2] = soucet;
		vybraneodpovedi[kolo][3] = pozice;
		soucet += PoleOdpovedi[soucet][pozice];
...
int main() {
	...
	std::string volba, nactenisouboru, ulozenisouboru, rng;
	...
	// Volba typu trenovani
	std::cout << std::endl;
	do {
		std::cout << "\nZadejte \"AI\" pro efektivni trenovani hrami sama se sebou nebo zadejte \"RNG\" pro standardni trenovani proti generatoru nahodnych cisel: ";
		std::cin >> rng;
		rng = MalePismena(rng);
	} while (rng != "ai" && rng != "rng");
